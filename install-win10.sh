...
wget -q --show-progress -O base_arm64-khanhnguyen.tar.7z.001 "http://192.168/base_arm64-khanhnguyen.tar.7z.001"
if [ -f base_arm64-khanhnguyen.tar.7z.001 ]; then
    wget -q --show-progress -O base_arm64-khanhnguyen.tar.7z.002 "http://192.168/base_arm64-khanhnguyen.tar.7z.002"
else
    echo "Error when download file"
    echo "Internet so bad!"
    exit 0
fi
wget -q --show-progress -O base_arm64-khanhnguyen.sha512sum "http://192.168/base_arm64-khanhnguyen.sha512sum" 2> /dev/null
